...
!MASSIMO MARTINA NEW VARIABLES - Cold Point Computation 06/12/2024
  integer:: tsize, jj, local_index
  real, allocatable, dimension(:) :: temp1,local_mins
  integer, allocatable, dimension(:):: temp2,local_mins_level
  integer, dimension(1) :: local_aux,local_aux_2
  allocate(local_mins(1),local_mins_level(1))
!END MASSIMO MARTINA
...
!******************************************************
!         write(*,*) "I AM GOING TO COMPUTE TROPOPAUSE"
...
51    continue
      ...
      !Computation of the Cold Point Tropopause - AirIntrusions Package - MM_19-06-2025

      tsize = 0
      jj=1
      do kz=kzmin,nuvz-1
           !Searching for temperature local minima
           if(tth(ix,jy,kz-1,n)>tth(ix,jy,kz,n) .and. tth(ix,jy,kz,n)<tth(ix,jy,kz+1,n)) then
              tsize = tsize + 1 !Increment the size of the local minima and height arrays
              !Dynamical allocation
              if(tsize > 1) then
                 !local temperature minima allocation
                 allocate(temp1(tsize))
                 temp1(:tsize) = local_mins 
                 call move_alloc(temp1,local_mins)
                 !heights (associated to the local temperature minima) allocation
                 allocate(temp2(tsize))
                 temp2(:tsize) = local_mins_level
                 call move_alloc(temp2,local_mins_level)

                 jj=jj+1 !index to fill in the array with temperature minima and their vertical levels
              end if
              !The temperature local minima and their
              !vertical levels are stored
              local_mins(jj) = tth(ix,jy,kz,n)
              local_mins_level(jj) = kz
           end if
      end do
      !Finding the location of the element with the lowest temperature
      local_aux = minloc(local_mins) !minloc() it finds the location of the element with the lowest temperature in the array
      local_aux_2 = local_mins_level(local_aux) !It stores the vertical levels associated to the lowest temperature
      local_index = local_aux_2(1) !Conversion from array to a simple integer
      !Tropopause height identification
      cold_tropopause(ix,jy,1,n)=zlev(local_index)
      if(cold_tropopause(ix,jy,1,n) == 0) then
         write(*,*) "cold tropopause: ", cold_tropopause(ix,jy,1,n)
         write(*,*) "local_mins_level", local_mins_level
         write(*,*) "local_mins", local_mins
         write(*,*) "tth", tth(ix,jy,kz,n)
         write(*,*) "local_aux", minloc(local_mins)
         write(*,*) "local_aux_2", local_mins_level(local_aux)
         write(*,*) "local_index", local_aux_2(1)
         write(*,*) "ix :", ix, "jy :", jy, "kz :", kz
         write(*,*) "nuvz :", nuvz
         write(*,*) "cold tropo height", cold_tropopause(ix,jy,1,n)
         write(*,*) "Lapse rate tropopause", tropopause(ix,jy,1,n)

         stop
      end if
        
    !End Cold Point Tropopause Computation - AirIntrusions Package
...
!AirIntrusions Modification - 12-04-2024
      !if ((iout.eq.4).or.(iout.eq.5).or.(mdomainfill.eq.2)) then !ORIGINAL VERSION
      if ((iout.eq.4).or.(iout.eq.5).or.(mdomainfill.eq.2).or.(mdomainfill.eq.1)) then !MM_12/04/2024
