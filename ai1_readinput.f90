...
!*************************************************
!AirIntrusion Modification - 20/06/2025
  !start read residence class - MASSIMO MARTINA CHANGES - 16-01-2025
  call skplin(1,unitpath)
  read(unitpath,*) nresidenceclass
  read(unitpath,*) lresidence(1)
  if(lresidence(1) .lt. 0 ) then
    write(*,*) ' #### FLEXPART MODEL ERROR RESIDENCE CLASSES #### '
    write(*,*) ' #### MUST BE GREATER OR EQUAL TO 0          #### '
    write(*,*) ' #### CHANGE SETTINGS IN      AGECLASSES.    #### '
    stop
  end if

  do i=2,nresidenceclass
     read(unitpath,*) lresidence(i)
     if (lresidence(i).le.lresidence(i-1) .OR. lresidence(i).lt.0) then
      write(*,*) ' #### FLEXPART MODEL ERROR RESIDENCE CLASSES #### '
      write(*,*) ' #### MUST BE GIVEN IN TEMPORAL ORDER.       #### '
      write(*,*) ' #### AND MUST BE GREATER OR EQUAL TO 0      #### '
      write(*,*) ' #### CHANGE SETTINGS IN      AGECLASSES.    #### '
      stop
     endif
  ...
  if(option_verbose .ge. 1) print*,'nresidenceclass',nresidenceclass

  if (nresidenceclass.gt.maxresidenceclass) then
    write(*,*) ' #### FLEXPART MODEL ERROR NUMBER OF RESIDENCE     #### '
    write(*,*) ' #### CLASSES GREATER THAN MAXIMUM ALLOWED.        #### '
    write(*,*) ' #### CHANGE SETTINGS IN  RESIDENCECLASSES OR      #### '
    write(*,*) ' #### RECOMPILE WITH LARGER MAXRESIDENCECLASS IN   #### '
    write(*,*) ' #### FILE PAR_MOD.F90                             #### '
    stop
  endif

  !start read source and tropopause methods
  call skplin(1,unitpath)
  read(unitpath,*) source_type
  read(unitpath,*) tropo_method
  if(option_verbose .ge. 1) then
     print*,'source_type',source_type
     print*,'tropo_method',tropo_method
  end if

  if(source_type .gt. 1 .or. source_type .lt. 0 ) then
      write(*,*) ' #### FLEXPART MODEL ERROR SOURCE_TYPE          #### '
      write(*,*) ' #### MUST BE GIVEN EQUAL TO 1 OR 0.            #### '
      write(*,*) ' #### CHANGE SETTINGS IN  INTRUSION OPTIONS.    #### '
      stop
  endif

  if(tropo_method .gt. 1 .or. tropo_method .lt. 0 ) then
      write(*,*) ' #### FLEXPART MODEL ERROR TROPO_METHOD          #### '
      write(*,*) ' #### MUST BE GIVEN EQUAL TO 1 OR 0.            #### '
      write(*,*) ' #### CHANGE SETTINGS IN  INTRUSION OPTIONS.    #### '
      stop
  endif
...
!****************************************
  ...
  if (outgrid_option .eq. 1 ) then !original code
!AirIntrusion Modification - 04/10/2025
!  if (outgrid_option .eq. 1 .or. outgrid_option .eq. 0) then
...
! Need to convert from lat/lon to grid-meters
    ...
    write(*,*) "outgrid_swlon",outgrid_swlon,"outgrid_swlat",outgrid_swlat
...
!*****************************************************************
 ...
 ! write(*,*) "xmet0", xmet0, "out_xm0", out_xm0, "xoutshift", xoutshift
 ! write(*,*) "ymet0", ymet0, "out_ym0", out_ym0, "youtshift", youtshift
 ! stop
