...
!*******************************************************************************
  ...
  use par_mod
  use com_mod
!AirIntrusions Modification - 31-07-2025
  use ai1_intrus_mod
...
!AirIntrusions Modification - 31-07-2025
  allocate(flux_crossing(2,0:numxgrid-1,0:numygrid-1, & !MM_10-10-2024
           1:maxpointspec_act),stat=stat)
  if (stat.ne.0) write(*,*)'ERROR: could not allocate tropo_crossing array ' !MM_10-10-2024

  allocate(sum_entering_time(2,0:numxgrid-1,0:numygrid-1, & !MM_10-10-2024
           1:maxpointspec_act),stat=stat)
  if (stat.ne.0) write(*,*)'ERROR: could not allocate sum_entering_time array ' !MM_10-10-2024

  allocate(sum_transition_time(2,0:numxgrid-1,0:numygrid-1, & !MM_10-10-2024
           1:maxpointspec_act),stat=stat)
  if (stat.ne.0) write(*,*)'ERROR: could not allocate sum_transition_time array ' !MM_10-10-2024

  !THERMODYN VARAIBALES - CHANGES 16-01-2025
  allocate(sum_pvtraj(2,0:numxgrid-1,0:numygrid-1, & !MM_16-01-2025
           1:maxpointspec_act),stat=stat)
  if (stat.ne.0) write(*,*)'ERROR: could not allocate potential vorticity array ' !MM_16-01-2025

  allocate(sum_wtraj(2,0:numxgrid-1,0:numygrid-1, & !MM_16-01-2025
           1:maxpointspec_act),stat=stat)
  if (stat.ne.0) write(*,*)'ERROR: could not allocate vertical velocity array ' !MM_16-01-2025

  allocate(sum_tketraj(2,0:numxgrid-1,0:numygrid-1, & !MM_16-01-2025
           1:maxpointspec_act),stat=stat)
  if (stat.ne.0) write(*,*)'ERROR: could not allocate turbulent kinetic energy array ' !MM_16-01-2025

  allocate(sum_pptraj(2,0:numxgrid-1,0:numygrid-1, & !MM_16-01-2025
           1:maxpointspec_act),stat=stat)
  if (stat.ne.0) write(*,*)'ERROR: could not allocate pressure array ' !MM_16-01-2025

  allocate(sum_ritraj(2,0:numxgrid-1,0:numygrid-1, & !MM_16-01-2025
          1:maxpointspec_act),stat=stat)
  if (stat.ne.0) write(*,*)'ERROR: could not allocate Richardson number array ' !MM_16-01-2025

  allocate(sum_eitraj(2,0:numxgrid-1,0:numygrid-1, & !MM_16-01-2025
           1:maxpointspec_act),stat=stat)
  if (stat.ne.0) write(*,*)'ERROR: could not allocate Ellrod number array ' !MM_16-01-2025

  allocate(sum_bvfsqtraj(2,0:numxgrid-1,0:numygrid-1, & !MM_16-01-2025
           1:maxpointspec_act),stat=stat)
  if (stat.ne.0) write(*,*)'ERROR: could not allocate brun vaisala frequency square array ' !MM_16-01-2025

  allocate(flux_mass(2,0:numxgrid-1,0:numygrid-1,1:nspec,1:maxpointspec_act), stat=stat)
  if(stat .ne. 0) write(*,*) 'ERROR: could not allocate mass flux array ' !MM_11-03-2025
!End AirIntrusions MOdification
