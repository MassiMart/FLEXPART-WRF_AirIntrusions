...
!  integer(kind=2),allocatable, dimension (:) :: cbt
  ...
  !AirIntrusions Modification 01-08-2025
  !BUG in the definition of xold and yold, they must be defined as real(kind=dp)
  !real :: drygridtotalunc,xold,yoldzold,xmassfract !ORIGINAL DEFINITION
  real :: drygridtotalunc,zold,xmassfract 
...
!AirIntrusions Modification 20/06/2025
  integer :: ll_index, mm_index
!END AirIntrusions Modification
...
!   endif
  ...
  allocate(npoint_id(maxpart),stat = stat)
  allocate(free_atm_entering_time(maxpart), stat = stat)
  allocate(tl_count_flag(maxpart),stat=stat)
  allocate(strato_count_flag(maxpart),stat=stat)

  !Also the array which are dynamically allocated must be intially 
  !allocate in the static way
  allocate(tl_particle_id(1),stat=stat)
  allocate(tl_transition_time(1),stat=stat)
  allocate(tl_entering_time(1,1),stat=stat)
  allocate(tl_residence_time(1,1),stat=stat)
  allocate(tl_flux_flag(1,1),stat=stat)
  allocate(tl_xcross(1,1), stat=stat)
  allocate(tl_ycross(1,1),stat=stat)
  allocate(tl_zcross(1,1),stat=stat)

  allocate(strato_particle_id(1),stat=stat)
  allocate(strato_transition_time(1),stat=stat)
  allocate(strato_entering_time(1,1),stat=stat)
  allocate(strato_residence_time(1,1),stat=stat)
  allocate(strato_flux_flag(1,1),stat=stat)
  allocate(strato_xcross(1,1), stat=stat)
  allocate(strato_ycross(1,1),stat=stat)
  allocate(strato_zcross(1,1),stat=stat)
  !THERMODYNAMIC VARIABLES
  !TL
  allocate(pvtraj_tl(1,1), stat=stat)
  allocate(wtraj_tl(1,1),stat=stat)
  allocate(tketraj_tl(1,1),stat=stat)
  allocate(pptraj_tl(1,1),stat=stat)
  allocate(ritraj_tl(1,1),stat=stat)
  allocate(eitraj_tl(1,1), stat=stat)
  allocate(bvfsqtraj_tl(1,1),stat=stat)
  allocate(topo_cross_tl(1,1),stat=stat)
  !STRATO
  allocate(pvtraj_st(1,1), stat=stat)
  allocate(wtraj_st(1,1),stat=stat)
  allocate(tketraj_st(1,1),stat=stat)
  allocate(pptraj_st(1,1),stat=stat)
  allocate(ritraj_st(1,1),stat=stat)
  allocate(eitraj_st(1,1), stat=stat)
  allocate(bvfsqtraj_st(1,1),stat=stat)
  allocate(topo_cross_st(1,1),stat=stat)

  !Initialization of some arrays
  tl_particle_id(1)     = -999
  strato_particle_id(1) = -999
  tl_residence_time(1,1) = -999
  strato_residence_time(1,1) = -999
  !END AirIntrusion Modification
...
!**********************************************************************
  !AirIntrusions Modification - 20/06/2025
  ll_index = 0 !Array index linked to "tl_*" arrays and "thermodyn" variables
  mm_index = 0 !Array index linkes to "strato_*" arrays
  colnum_tl   = expected_crossing !initialize the number of columns for the arrays involved in
                                  !the identification of the intrusions
  colnum_st   = expected_crossing
  !END AirIntrusions Modification 
...
!***********************************************************************
  ...
  ! Nested output grid
                     ...
                     end do !end do over nage
                 endif
              end do !end do over kp
             end do !end do over ks
         endif
...
!!     print*,xtra1(j),th_xtra1,OMP_GET_THREAD_NUM()
            ...
            endif !end if itra1(jj) .eq. jtime

            !AirIntrusions Modification - 20/06/2025
            !write(*,*) "I AM IN AI1 TIME MANAGER SERIAL BEFORE ENTERING UP INTRUSION IDENTIFIER"
            !write(*,*) "size tl_particle_id", size(tl_particle_id),"allocated", allocated(tl_particle_id)
            !write(*,*) "tl_particle_id", tl_particle_id
            !write(*,*) "*********************************************************************************"

            call up_intrusion_identifier(jtime,jj,xold,yold,zold,ll_index,mm_index)
            !END AirIntrusions Modification

         end do !loop over particles
     endif !end if numpart
...
! update the drydepo
  ...
  !AirIntrusions Modification - 20/06/2025
  if(jtime == ideltas) then
    call up_intrusionoutput(jtime,nage)
  end if
  !END AirINtrusion Modification - 20/06/2025
...
!  deallocate(xmasssave)
  ...
  !AirIntrusion Modification - 20/06/2025
  deallocate(free_atm_entering_time,tl_transition_time,&
             strato_transition_time, tl_count_flag,&
             strato_count_flag, tl_entering_time,&
             strato_entering_time,tl_residence_time,&
             strato_residence_time, tl_flux_flag,&
             strato_flux_flag,pvtraj_tl,wtraj_tl,tketraj_tl,&
             pptraj_tl,ritraj_tl,eitraj_tl,bvfsqtraj_tl,topo_cross_tl,&
             pvtraj_st,wtraj_st,tketraj_st,pptraj_st,ritraj_st,eitraj_st,&
