...
! https://github.com/MassiMart/FLEXPART-WRF_MolavePaper                        *
!                                                                              *
! 06 August 2025                                                                 *
!*******************************************************************************
...
!AirIntrusions Modification - 06-08-2025
!      if (turb_option .eq. turb_option_tke .or. & !ORIGINAL VERSION
!          turb_option .eq. turb_option_mytke) then !ORIGINAL VERSION
!-
     if (turb_option .eq. turb_option_tke .or.  & ! NEW VERSION
           turb_option .eq. turb_option_mytke .or. & !Since turb_option_tke = 2 and turb_option_mytke = 3 (defined in par_mod.f90) 
           turb_option .eq. 1) then  !Addition of this line in order to read the TKE value also when turb_option is set to 1 in flexwrf.input
...
!   read the data into k=kbgn,nuvz
      varname = 'TKE'
!AirIntrusions MOdification - 31-07-2025
 !Massimo Martina reported the presence of a BUG 05-03-2025 in readwind.f90:
  !the WRF output file contains "nuvz" levels for TKE and NOT "(nuvz-add_sfc_level)
  !Hence, he suspects that also here "nuvz-1" should be replace by "nuvz". It needs 
  !to be testes!
      !lendim_exp(3) = nuvz-1 !ORIGINAL VERSION
      lendim_exp(3) = nuvz
      lendim_max(3) = nuvzmax
      call read_ncwrfout_1realfield( ierr, idiagaa, fnamenc, &
...
!AirIntrusions Modification - 31-07-2025
!CAPE
      if(cape_option .eq. 1) then
        write(*,*) "Read CAPE", "cape_otion",cape_option
        varname = 'CAPE'
        lendim_exp(3) = nuvz -add_sfc_level
        lendim_max(3) = nuvzmax
        call read_ncwrfout_1realfield( ierr, idiagaa, fnamenc, &
             varname, capehn(0,0,kbgn,n,l), &
             itime, &
             ndims, ndims_exp, ndims_max, &
             lendim, lendim_exp, lendim_max )

        if (ierr .ne. 0) then
            print*,'NO CAPE available.'
            write(*,*) "ierr", ierr
            stop
        end if
